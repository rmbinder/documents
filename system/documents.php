...
foreach ($documents as $document) {
    ...
    if ($serialNumberOfMember == substr($serialNumberOfFile, 0, strlen($serialNumberOfMember))) {
        $templateRow = array();
        $templateRow['url'] = SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_MODULES . '/documents-files.php', array(
            'mode' => 'download',
        ...
        $templateRow['actions'][] = array(
            'url' => SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_MODULES . '/documents-files.php', array(
                'mode' => 'download',
                'file_uuid' => $document['fil_uuid']
            )),
            'icon' => 'bi bi-download',
        ...
        $templateRow['actions'][] = array(
            'dataHref' => 'callUrlHideElement(\'row_' . $document['fil_uuid'] . '\', \'' . SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_MODULES . '/documents-files.php', array(
                'mode' => 'file_delete',
                'file_uuid' => $document['fil_uuid']
            )) . '\', \'' . $gCurrentSession->getCsrfToken() . '\')',
            'dataMessage' => $gL10n->get('SYS_DELETE_ENTRY', array(
                $document['fil_name']
            )),
            'icon' => 'bi bi-trash',
...
$page->assignSmartyVariable('urlDocumentsPreferences', SecurityUtils::encodeUrl(ADMIDIO_URL . FOLDER_PLUGINS . PDS_PLUGIN_FOLDER . '/system/preferences.php'));
